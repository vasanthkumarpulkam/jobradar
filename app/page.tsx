...
function statNumStyle(color: string): React.CSSProperties {
  return { fontSize: 26, fontWeight: 700, color, lineHeight: 1 };
}
function btnStyle(active: boolean): React.CSSProperties {
  return {
    ...
    fontSize: 10, fontFamily: "inherit", letterSpacing: 0.8,
...
export default function Page() {
  ...
  return (
    <div style={{ fontFamily:"'DM Mono','Fira Code',monospace", background:"#05080f", minHeight:"100vh", color:"#c9d1d9" }}>
          ...
          <div>
            <div style={{ display:"flex", alignItems:"center", gap:10 }}>
              ...
              <span style={{ fontSize:9, padding:"2px 8px", letterSpacing:1.2,
                             ...
                             color:"#3fb950", borderRadius:3 }}>LIVE</span>
            </div>
          ...
          <div style={{ display:"flex", alignItems:"center", gap:18 }}>
            {stats.map(([n, c, l]) => (
              <div key={l} style={{ textAlign:"center" }}>
                <div style={statNumStyle(c)}>{n}</div>
      ...
      {/* ── SOURCE BAR ── */}
      {meta && (
        <div style={{ display:"flex", padding:"0 24px", borderBottom:"1px solid #0d1117", background:"#080c14" }}>
          ...
          <div style={{ padding:"8px 18px", display:"flex", alignItems:"center", gap:8 }}>
            <div style={{ width:6, height:6, borderRadius:"50%", background:"#e3b341", boxShadow:"0 0 6px #e3b341" }}/>
            <span style={{ fontSize:10, color:"#484f58" }}>Remote</span>
            <span style={{ fontSize:13, fontWeight:700, color:"#e3b341" }}>{jobs.filter(j=>j.remote).length}</span>
          </div>
        </div>
      )}
      ...
      <div style={{ padding:"14px 24px", maxWidth:1000, margin:"0 auto" }}>
        ...
        {loading && jobs.length === 0 && (
          <div style={{ textAlign:"center", padding:"80px 20px" }}>
            <div style={{ fontSize:11, color:"#3fb950", letterSpacing:3, marginBottom:16 }}>
        ...
        {!loading && filtered.length === 0 && lastFetch && (
          <div style={{ textAlign:"center", padding:"60px 20px", color:"#30363d" }}>
            <div style={{ fontSize:13, marginBottom:8 }}>NO JOBS IN LAST 30 MINUTES</div>
        ...
        {filtered.map((job, i) => {
          ...
          return (
            <div key={job.id}
              style={{ ...cardStyle(isNew), borderLeft:`3px solid ${rc}` }}
        ...
        {lastFetch && jobs.length > 0 && (
          <div style={{ textAlign:"center", padding:"20px 0 28px", fontSize:10, color:"#21262d", letterSpacing:1 }}>
            {filtered.length} JOBS · {meta?.scanned} COMPANIES SCANNED · NEXT SCAN {mins}:{secs}
            <br/>
